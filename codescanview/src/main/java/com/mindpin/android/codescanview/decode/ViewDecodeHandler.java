 ...
 * 描述: 接受消息后解码
 */
final class ViewDecodeHandler extends Handler {

    CodeScanView view = null;

    ViewDecodeHandler(CodeScanView activity) {
        this.view = activity;
    }
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        String result = manager.decode(rotatedData, width, height, true, view.getCustomX(), view.getCustomY(), view.getCropWidth(),
                view.getCropHeight());

        if (result != null) {
            if (view.isNeedCapture()) {
                // 生成bitmap
                PlanarYUVLuminanceSource source = new PlanarYUVLuminanceSource(rotatedData, width, height, view.getCustomX(), view.getCustomY(),
                        view.getCropWidth(), view.getCropHeight(), false);
            ...
            if (null != view.getHandler()) {
                view.getHandler().sendEmptyMessage(R.id.decode_failed);
            }
        }
    }
